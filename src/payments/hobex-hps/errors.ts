...
export class HpsConnectTerminalError extends HpsConnectException {
   ...
   * ungemessen, siehe [NOT_FOUND_HTTP_STATUS] und den Dart-Zwilling
   * (`HpsHttpException.isNotFound`): es kann "diesen Vorgang kenne ich nicht"
   * heissen oder "diesen Endpunkt gibt es hier nicht". `payments.ts` benennt
   * beim Abbruch beide Lesarten, statt eine zu behaupten.
   *
   * Liest, wie [isTerminalBusy], bevorzugt [terminalHttpStatus] und faellt nur
   * ohne dieses Feld auf den Meldungstext zurueck.
   */
