 ...
 * ein vorheriger fehlt (`fon_missing`, `vertrag_offen`, `signature_not_ready`).
...
export const PARTNER_ABLAUF: readonly AblaufSchritt[] = [
  ...
  },
  {
    key: 'avv',
    text:
      'Auftragsverarbeitungsvertrag (Art. 28 DSGVO). Je nach Vertragsweg des Partner-Kontos: ' +
      'der Betrieb bestaetigt selbst (direkt), der Partner meldet in Vollmacht (vollmacht), ' +
      'oder der Vertrag liegt beim Partner (unterauftrag).',
    aufruf: 'reportCustomerVertrag',
    wartetAuf: 'customer.avv_accepted',
    fehltCode: 'vertrag_offen',
  },
  {
    key: 'signatur',
 ...
 *
 * **Bewusst ohne Vertragsschritt:** ob der Auftragsverarbeitungsvertrag
 * vorliegt, sagt der Kundenstatus nicht (es gibt keinen eigenen Status dafuer).
 * Wer ihn wissen will, hoert auf `customer.avv_accepted` — oder erfaehrt es an
 * der Kasse, die mit `vertrag_offen` nicht live geht.
 */
